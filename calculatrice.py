p_nbr = int(input("Entrez un premier nombre : "))
d_nbr = int(input("Entrez un deuxième nombre : "))

résultat = f"Le résultat de l'addition de {p_nbr} avec {d_nbr} est égal à {p_nbr + d_nbr}"

print(résultat)

'''
Réalisation plus tard : 
Faire en sorte que si l'utilisateur n'écrit pas un chiffre cela annule le résultat et redemande à l'utilisateur
de marquer un chiffre (avec une boucle)
'''